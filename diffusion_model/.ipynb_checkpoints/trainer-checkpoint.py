...
# chekcing if we cause the mixed precesion or not  
try:
...
# help to provide the data continous in cycle  
def cycle(dl):
    while True:                  # loop again to the datset 
        for data in dl:          # pass to the dataset  
            yield data
...
'''
num_to_groups(10, 3)
it returns a vecotr [3,3,3,1]
'''

##########################################
#  updating the weights  by backpropagation . if we are using the fp16 the loss and small precsion values wil be lost so we need to scale them and then down scale reuse the gredient  
...
# this is to keep the average version of the model weights over the training  
# EMA(t) =βxEMA(t-1)+(1-β)xnew
class EMA():
...
""
# this is to create a noise of the same shape as the our data 
# there  is option for to control should all the noise should be same or differnt for each data in the batch 
...
######################################################################
# time steps containt the total steps
#  s is some offset we used to find the value of aplha 
# alpha is calculted based on the eqution  alphas_cumprod = np.cos(((x / steps) + s) / (1 + s) * np.pi * 0.5) ** 2
# noramlize the value of the alpha so that the first value of the alpha  is 1  this  is to indicate that at step 0 we are using the pure image  
# equztion find the beta from alpha which controls how mmuch noise is added in each time stamp  
# when it returns it check if the beta have negative values of larger than  0.999 if less than 0 we replcae it with 0 ,if greter we replace it    with  0.999 on returning  
# ??  how the line spce , eqution of alpha and b  
def cosine_beta_schedule(timesteps, s = 0.008):
    ...
    return np.clip(betas, a_min = 0, a_max = 0.999)      # why clip it will alwasu give the value less than 1 and greter than  0  ??
    # it returns the  b shedule for 1 to T  steps each value of betas[i] control the amount of noise added on the step i  
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        # if the beta is given use it or else genrate it  
        if exists(betas):
            betas = betas.detach().cpu().numpy() if isinstance(betas, torch.Tensor) else betas
        else:
            betas = cosine_beta_schedule(timesteps)

        alphas = 1. - betas      # it alpha[i]  control how much data is retained at timepstep i form i-1  
        alphas_cumprod = np.cumprod(alphas, axis=0)    # it controls how much data is retained at timestep i from the 0  
        alphas_cumprod_prev = np.append(1., alphas_cumprod[:-1])   #it gives previous  cumulative value like how much was added up to i-1  

        timesteps, = betas.shape 
        self.num_timesteps = int(timesteps)
        self.loss_type = loss_type

        to_torch = partial(torch.tensor, dtype=torch.float32)   ## WHAT IS THIS  ????????????//
 
        # Storing these values to gpu :
...
class Trainer(object):
    ...
    def create_log_dir(self):
        ...
        return log_dir
    # to copy the parmater value form the model to ema model  exactly in the early state when step is less than ema_start
    def reset_parameters(self):
        self.ema_model.load_state_dict(self.model.state_dict())
    # if step is less than ema_start we copy the same modle else we do the average update for the emamodly usign the beta 
    def step_ema(self):
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            print(f'[Step {self.step}] Avg Loss: {average_loss:.4f} | Step Time: {step_time:.2f}s | Total Time: {elapsed_time:.2f}s')
